...
const BoardView = ({ filteredTasks, searchQuery, filterPriority, groupBy = 'status' }: BoardViewProps) => {
  const { tasks, setTasks, taskStatuses } = useAppContext();
        ...
        return (
          ...
          >
            ...
            <div className="flex items-center justify-between px-4 py-4 sticky top-0 bg-transparent z-10 backdrop-blur-sm rounded-t-2xl">
              ...
              <div className="flex items-center gap-1 opacity-0 group-hover:opacity-100 transition-opacity" style={{ opacity: 1 }}> {/* Keeping visible for better UX, usually it's nice to always see the plus */}
                {groupBy === 'status' && (
                  <button
                ...
                {columnTasks.map(task => {
                  ...
                  return (
                    <motion.div
                      ...
                      className={`relative bg-[#141414] border rounded-xl p-4 cursor-grab active:cursor-grabbing transition-all duration-200 group overflow-hidden
                        ${isHovered ? 'border-gray-600 shadow-[0_8px_30px_rgba(0,0,0,0.5)] -translate-y-0.5' : 'border-white/5 shadow-sm'}
                      `}
                    >
                      ...
                      {/* Info Row: Subtasks, Comments, Due Date */}
                        ...
                        <div className="flex items-center gap-1.5">
                          {task.priority !== 'None' ? (
                            <div className="flex items-center gap-1.5 text-[11px] font-bold tracking-wide px-2 py-1 rounded-md bg-white/5" style={{ color: prio.color }}>
                             ...
                             <div className="w-5 h-5" /> // placeholder to maintain height
                          )}
                        </div>
